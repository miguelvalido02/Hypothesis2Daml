...
tokens_fixed = ["USD", "BTC", "ETH"] # Add more tokens if desired
...
def lend(cid: str, user: str, token: str, amount: Decimal) -> str:
    res = make_request("exercise", act_as=user, template_id=BAL_TID, contract_id=cid,
                       choice="Lend", argument={"user": user, "token": token, "amount": str(amount)}) #Add or remove args if needed
    return res["exerciseResult"]

def withdraw(cid: str, user: str, token: str, amount: Decimal) -> str:
    res = make_request("exercise", act_as=user, template_id=BAL_TID, contract_id=cid,
                       choice="Withdraw", argument={"user": user, "token": token, "amount": str(amount)}) # Add or remove args if needed
    return res["exerciseResult"]
...
def borrow(cid: str, user: str, collateral_token: str, collateral_amount: Decimal,
    ...
    res = make_request("exercise", act_as=user, template_id=BAL_TID, contract_id=cid,
                       ...
                       #Add or remove args if needed
    return res["exerciseResult"]

def repay(cid: str, user: str, collateral_token: str, borrow_token: str) -> str:
    res = make_request("exercise", act_as=user, template_id=BAL_TID, contract_id=cid,
                       choice="Repay",
                       argument={"user": user, "collateralToken": collateral_token, "borrowToken": borrow_token}) #Add or remove args if needed
    return res["exerciseResult"]
...
def get_balances(cid: str, user: str) -> dict[str, Decimal]:
    ...
    for item in res["exerciseResult"]:
        #Process result with keys "_1" and "_2" if needed (Depends on the implementation)
...
# Check if withdraw respects lender balance
@given(tok=alpha, a1=money, a2=money)
@settings(max_examples=12, deadline=None)
...
# Check if borrow and repay work correctly in a roundtrip
