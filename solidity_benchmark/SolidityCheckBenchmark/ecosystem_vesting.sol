...
//0xd05d6f1656182da717fad7e4eeabab4ffd49f609.sol
contract Ownable {
...
contract EcosystemVesting is Ownable {
    ...
    uint256 public constant MaximumHoldingPeriod = 630 days;

    uint256 public constant EcosystemCap = 54100000 ether; // 54.1 million tokens
    ...
    event EcosystemVestingInitialized(
        address _to,
        uint256 _tokens,
        uint256 _startTime
    );

    event EcosystemVestingUpdated(
        address _to,
    ...
    function claimTokens(
        address beneficiary
    ) external onlyOwner returns (uint256 tokensToClaim) {
        ...
        if (now.sub(startTime) >= MaximumHoldingPeriod) {
            ...
            uint256 tokensNotToClaim = (
                holdings[beneficiary].tokensCommitted.mul(1000 - permill)
            ).div(1000);
    ...
    function calculatePermillToRelease(
        uint256 _startTime
    ) internal view returns (uint256 permill) {
    ...
    function initializeVesting(
        ...
        totalEcosystemTokensCommitted = totalEcosystemTokensCommitted.add(
            _tokens
        );
        ...
        if (holdings[_beneficiary].tokensCommitted != 0) {
            holdings[_beneficiary].tokensCommitted = holdings[_beneficiary]
                .tokensCommitted
                .add(_tokens);

            holdings[_beneficiary].tokensRemaining = holdings[_beneficiary]
                .tokensRemaining
                .add(_tokens);
            ...
            holdings[_beneficiary] = Holding(_tokens, _tokens, _startTime);
