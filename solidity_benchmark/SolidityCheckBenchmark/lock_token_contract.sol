...
//0x0af0e0d19f08e0a9c141ae98fe6ecc1737f3192f.sol
contract Token {
...
contract LockTokenContract {
    ...
    address public FoundationAddress =
        address(0x98d7cbfF0E5d6807F00A7047FdcdBDb7B1192f57);

    address public TeamAndAdviserAddreesOne =
        address(0xb89b941F7cd9eBCBcAc16cA2F03aace5cf8e2edc);

    address public TeamAndAdviserAddreesTwo =
        address(0x5a403e651EC2cD3b6B385dC639f1A90ea01017f7);

    address public GubiTokenAddress =
    ...
    function() external payable {
        require(
            msg.sender == FoundationAddress ||
                msg.sender == TeamAndAdviserAddreesOne ||
                msg.sender == TeamAndAdviserAddreesTwo
        );
        ...
        if (amount > 0) {
            AddressWithdrawals[msg.sender] =
                AddressWithdrawals[msg.sender] +
                amount;

            require(token.transfer(msg.sender, amount * 1e10));
        }
    }
    ...
    function calculateUnlockAmount(
        ...
        uint phase = _now - StartTime / StageSection;

        if (phase >= stage.length) {
            phase = stage.length - 1;
        }

        uint256 unlockable = stage[phase] - AddressWithdrawals[msg.sender];

        if (unlockable == 0) {
            return 0;
        }

        if (unlockable > _balance / 1e10) {
            return _balance / 1e18;
        }
