...
//0x255afc55ef255cfb86d89a9650baf18c6d05265c.sol
/**
...
contract MultiVesting is Ownable, Destroyable {
    ...
    function release(address _beneficiary) private isBeneficiary(_beneficiary) {
    ...
    function addBeneficiary(
        ...
        bool _revocable,
        string _description
    ) public onlyOwner isNotBeneficiary(_beneficiary) {
        require(_beneficiary != address(0));

        require(_cliff >= _start);

        require(
            token.balanceOf(this) >= totalVested.sub(totalReleased).add(_vested)
        );
    ...
    function releasableAmount(
        ...
        return
            vestedAmount(_beneficiary).sub(
                beneficiaries[_beneficiary].released
            );
    }
    ...
    function vestedAmount(address _beneficiary) public view returns (uint256) {
        ...
        if (now < beneficiary.cliff) {
            ...
            now >= beneficiary.start.add(beneficiary.duration) ||
            beneficiary.revoked
        ) {
            return totalBalance;
        } else {
            return
                totalBalance.mul(now.sub(beneficiary.start)).div(
