...
//0xdab8aea5896181210fb7cf1a95b4513b077240f4.sol
contract Coinflip {
    enum WagerState {
        emptyRoom,
        wagerMade,
    ...
    function start(uint256 _address) private {
        ...
        if (games[_address].isPlayerOneEagle == true && coinFlip <= 49) {
            ...
            games[_address].player1.transfer(
                games[_address].wager *
                    2 -
                    ((games[_address].wager * 2) * percent) /
                    1000
            );
        } else if (
            games[_address].isPlayerOneEagle == false && coinFlip >= 50
        ) {
            games[_address].winner = games[_address].player1;
            ...
            games[_address].player2.transfer(
                games[_address].wager *
                    2 -
                    ((games[_address].wager * 2) * percent) /
                    1000
    ...
    function changePercent(uint256 newPercent) public onlyOwner {
        percent = newPercent;
    }

    function changeMinimumBet(uint256 newMinimumBet) public onlyOwner {
    ...
    function getCurrentState(
        uint256 _address
    ) public view returns (WagerState) {
